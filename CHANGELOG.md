...
OEM bundle bumps to 13 (new VBS files staged into `C:\OEM\`); needs a container recreate (or `winpodx app run desktop` to trigger first-boot reapply on a fresh install) to take effect on existing pods. Existing 0.3.0-RTM1 pods keep working — they just retain the brief PS flash on agent autostart and UWP launches until recreated.
